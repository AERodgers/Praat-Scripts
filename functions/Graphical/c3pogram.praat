...
procedure drawC3Pogram: .pitchTable, .secondParam, .minT, .maxT,
    ... .minF0, .maxF0, .type, .hz_ST
    ...
    Append column: "shade"
    Formula: "shade",
        ... "1 - (self[""value""] - .minPar2) / (.maxPar2 - .minPar2)"
    ...
    for .i to .numPitchPts
        ...
        if not(abs(.shT - .curT)*1000 > 5.5555)
            Paint circle: "{'.sh','.sh',1-0.8*'.sh'}",
                ... .curT, .curF0, .di * 0.1 + .di * (1 - .sh)
            Colour: "blue"
            Line width: 0.5
...
procedure pitch: .sound, .minF0, .maxF0
    selectObject: .sound
    .obj = To Pitch (ac): 0, .minF0, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14,
        ... .maxF0
endproc
...
procedure intensity: .sound, .minF0, .minT, .maxT
    ...
    selectObject: .table
    Formula: "value", "self[""value""] - (self[""time""]
...
procedure cpp: .sound, .minF0, .maxF0, .pitchTable
    ...
    for .i to .numRows
        ...
        .cpp[.i] = Get peak prominence: .minF0, .maxF0, "Parabolic", 0.001, 0,
            ... "Straight", "Robust"
        Remove
    endfor
